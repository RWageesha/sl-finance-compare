...
onMounted(async () => {
  try {
    rows.value = (await fetchLoans().catch(() => [])).filter((r) => r.category_code === 'GOLD_LOAN')
  } finally {
    loading.value = false
  }
})
...
const filtered = computed(() => {
  ...
  return [...matched].sort((a, b) => (sortBy.value === 'rate_asc' ? a.interest_rate - b.interest_rate : b.interest_rate - a.interest_rate))
})

function productLabel(r: ProductRate): string {
  return r.rate_label || r.tenure_label || formatCategoryLabel(r.category_code)
}
</script>
      ...
      <div class="wrap">
        ...
        <div class="results-header">
          <span class="count">
            ...
            <template v-else>{{ filtered.length }} product{{ filtered.length === 1 ? '' : 's' }} found</template>
          </span>
        ...
        <p v-if="!loading && filtered.length === 0" class="empty">No gold loan products match.</p>
